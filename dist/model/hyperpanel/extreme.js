...
function ensureZzzSkills(avatar) {
...
function pickTopSubStats(weights, mainPropId) {
...
function buildExtremeEquipList(equipList, weights) {
    const out = [];
    ...
    for (const equip of list) {
        ...
        const properties = picked.map((propId) => {
            ...
            const base = Number(baseValueData?.[String(propId)] || 0) * rolls;
            return {
...
async function readPanelFile(uid) {
...
export async function updateExtremePanelsFromPanelDb({ targetUid = '10000000' } = {}) {
    ...
    const { ZZZAvatarInfo } = (await import('../avatar.js'));
    ...
    if (!uidList.length) {
        return {
            ok: false,
            message: '未找到可用的面板数据源：请先让一些用户更新/刷新一次面板，生成 plugins/ZZZ-Plugin/data/panel/<uid>.json',
        };
    }
    ...
    for (const uid of uidList) {
        ...
        for (const ds of list) {
            ...
            if (!Array.isArray(equipList) || equipList.length <= 0)
                continue;
            scannedAvatars++;
            let equipScore = 0;
            try {
    ...
    if (!bestByChar.size) {
        return {
            ok: false,
            message: '扫描完成但未找到任何有效角色面板：可能面板数据为空/不完整。',
        };
    }
    const out = [];
    const now = Date.now();
    const sorted = _.orderBy(Array.from(bestByChar.entries()).map(([id, v]) => ({ id, ...v })), ['equipScore', 'id'], ['desc', 'asc']);
