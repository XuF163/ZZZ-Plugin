const { Panel } = await import('./dist/apps/panel.js').catch(() =>
  import('./apps/panel.js')
)

const settings = (await import('./dist/lib/settings.js').catch(() =>
...
export default class Button extends Panel {
  constructor() {
    super({
      ...
      rule: [
        {
          reg: '#绝区零更新面板|#绝区零面板更新|#绝区零刷新面板|#绝区零面板刷新|#绝区零更新展柜面板|#绝区零展柜面板更新$',
          fnc: 'profile1',
        },
  ...
  async profile1(e) {
    ...
    const { roleList } = await this.getUserData()
    const button = []
    ...
    const staticList = [
      ...
      {
        label: '爱发电',
        link: settings.getConfig('config').donationLink || 'https://afdian.com',
      },
    ]
  ...
  async getUserData() {
    ...
    try {
      ...
      logger.debug('获取UID失败，使用默认按钮:', error?.message || error)
    }
