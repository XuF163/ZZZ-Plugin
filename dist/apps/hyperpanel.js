...
function toAvatarList(ds) {
    ...
    if (!av)
        return [];
    if (Array.isArray(av))
        return av.filter(Boolean);
    if (typeof av === 'object')
        return Object.values(av).filter(Boolean);
    return [];
}
function pickAvatarByName(list, name) {
    ...
    if (!target)
        return null;
    const exact = list.find((a) => a?.name_mi18n === target ||
        a?.name === target ||
...
export class HyperPanel extends ZZZPlugin {
    ...
    async generateBunPreset() {
        if (HyperPanel.locked)
            return { ok: false, message: '正在生成中，请稍后再试…' };
    ...
    async showHyperPanel() {
        ...
        if (!picked) {
            if (!list.length) {
                await this.reply(`极限面板数据获取失败：${apiErr?.message || apiErr || '数据为空'}\n` +
            ...
            const hints = list
                ...
                .slice(0, 10);
            await this.reply(`未找到角色「${rawName}」的极限面板。\n` +
                (hints.length
                    ? `你是不是想查：${hints.join('、')}`
                    : '可尝试使用角色全名/常用别名再试。'));
            return true;
        ...
        if (handler?.has?.('zzz.tool.panel')) {
            try {
                ...
                await this.reply(`极限面板数据已获取，但渲染失败：${e?.message || e}\n` +
        ...
        try {
            ...
            if (typeof Panel !== 'function')
                throw new Error('panel tool not available');
...
//# sourceMappingURL=hyperpanel.js.map