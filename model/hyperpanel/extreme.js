...
function pickTopSubStats(weights, mainPropId) {
  const all = Object.keys(baseValueData || {}).map((k) => Number(k)).filter((n) => Number.isFinite(n))
...
export async function updateExtremePanelsFromPanelDb({
  ...
  if (!uidList.length) {
    return { ok: false, message: '未找到可用的面板数据源：请先让一些用户更新/刷新一次面板，生成 plugins/ZZZ-Plugin/data/panel/<uid>.json' }
  }
  ...
  if (!bestByChar.size) {
    return { ok: false, message: '扫描完成但未找到任何有效角色面板：可能面板数据为空/不完整。' }
  }
