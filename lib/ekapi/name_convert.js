//从api缓存文件加载
// name_convert.js (基于 Hakush 数据重构)
...
// import { updateHakushData } from './Hakush.js'; // 如果需要自动更新 Hakush 数据，取消此行注释
...
// 旧版 avatars.json 数据路径
const OLD_DATA_PATH = path.join(__dirname, '..', '..', 'resources', 'enka'); // 假设旧 avatars.json 在这里
const AvatarIconDataFile = 'avatars.json';

// 图片基础 URL
const BASE_IMAGE_URL = 'https://enka.network'; // 用于拼接图片 URL

// --- 缓存 ---
const characterCache = new Map(); // Hakush 角色数据缓存
const weaponCache = new Map();    // Hakush 武器数据缓存

// --- 日志记录器 (使用 console 作为默认) ---
...
const ID_TO_EN = {
    ...
    '30503': 'SpRecover', '30501': 'SpRecover', // 固定值回复
    '30502': 'SpRecoverPercent', // 百分比能回
...
const PERCENT_ID_LIST = Object.keys(ID_TO_PROP_NAME)
    .filter(id =>
        ...
        ['20103', '20101', '21103', '21101', '23103', '23101', '30502'].includes(id) // 明确是百分比的 ID
    );
...
// 驱动盘主词条每阶成长值 (基于 Hakush 数据或推测，需验证)
...
function getCharBasicInfo(char_id) {
    ...
    const professionKey = Object.keys(data.WeaponType || {})[0]; // WeaponType 在 Hakush 里存的是职业
...
function constructIconUrl(relativePath) {
    ...
    return `${BASE_IMAGE_URL}/ui${finalPath}`; // e.g., https://enka.network/ui/Sprite/...
}
...
function get_char_circle_icon_url_from_avatars(char_id) {
    ...
    const iconPathField = 'CircleIcon'; // avatars.json 中圆形图标的字段名
    ...
    if (avatar_icon_data[charIdStr]?.[iconPathField] && typeof avatar_icon_data[charIdStr][iconPathField] === 'string') {
        ...
        // 假设 avatars.json 中的路径格式为 "/zzz/IconRoleCircleXX.png"
        if (iconPath.startsWith('/')) {
            return BASE_IMAGE_URL + iconPath; // 直接拼接
        } else {
            logger.warn(`[AvatarsURL] Unexpected path format in ${AvatarIconDataFile} for char ${charIdStr}, field ${iconPathField}: "${iconPath}".`);
            return `${BASE_IMAGE_URL}/${iconPath}`; // 尝试拼接
        }
    } else {
        // logger.warn(`[AvatarsURL] ${iconPathField} missing in ${AvatarIconDataFile} for char ${charIdStr}.`);
        return undefined; // 返回 undefined 表示未找到
    }
}
...
function get_char_base_icon_url_from_avatars(char_id) {
    const charIdStr = String(char_id);
    // !! 重要: 需要根据你的 avatars.json 文件确认方形图标的实际字段名 !!
    const iconPathField = 'IconPath'; // 假设字段名为 'IconPath' 或 'BaseIconPath' 或 'SquareIcon' 等
    ...
    if (avatar_icon_data[charIdStr]?.[iconPathField] && typeof avatar_icon_data[charIdStr][iconPathField] === 'string') {
        ...
        // logger.warn(`[AvatarsURL] Base icon path (field: ${iconPathField}) missing in ${AvatarIconDataFile} for char ${charIdStr}.`);
        return undefined;
    }
}
...
export {
    // 加载的数据
    hakush_equip_data,          // Hakush 驱动盘数据
    avatar_icon_data,           // 旧版头像数据 (来自 avatars.json)
    ...
    // 工具函数
    getCharBasicInfo,                       // Hakush 角色基础信息
    constructIconUrl,                     // Hakush 资源 URL 构造器 (武器/驱动盘)
    calculateCharBaseStat,                // Hakush 角色属性计算
    render_weapon_detail,                 // Hakush 武器属性计算
...
// import { updateHakushData } from './Hakush.js'
//
//
// const __filename = fileURLToPath(import.meta.url);
// const __dirname = path.dirname(__filename);
...
//
// //
// const ENKA_DATA_PATH = path.join(__dirname, '..','..', 'resources', 'enka');
// const MAP_PATH = ENKA_DATA_PATH;
// const CHAR_ALIAS_FILE = 'char_alias.json'
...
//     }
// } catch (error) {
//     logger.error(`读取或解析 ${SkillParamFile} 时出错:`, error);
//
//     PartnerId2SkillParam = {};
// }
//
// let char_alias_data = {};
//  const charAliasPath = path.join(MAP_PATH, CHAR_ALIAS_FILE);
// try {
//
//     if ( fs.existsSync(charAliasPath)) {
//          const charAliasContent = fs.readFileSync(charAliasPath, { encoding: 'utf-8' });
//         char_alias_data = JSON.parse(charAliasContent);
//     } else {
//         logger.warn(`Alias file not found at: ${CHAR_ALIAS_FILE}. Aliases will not be loaded.`);
//     }
// } catch (error) {
//     logger.error('Error reading or parsing char_alias.json:', error);
// }
//
// let partner_data = {};
// try {
...
//     } else {
//          logger.error(`Partner data file not found at: ${partnerDataPath}`);
//     }
// } catch (error) {
//     logger.error(`Error reading or parsing ${PartnerId2DataFile}:`, error);
// }
//
// let weapon_data = {};
// try {
...
//     } else {
//          logger.error(`Weapon data file not found at: ${weaponDataPath}`);
//     }
// } catch (error) {
//     logger.error(`Error reading or parsing ${WeaponId2DataFile}:`, error);
// }
//
// let equip_data = {};
// try {
...
//     } else {
//          logger.error(`Equipment data file not found at: ${equipDataPath}`);
//     }
// } catch (error) {
//     logger.error(`Error reading or parsing ${EquipId2DataFile}:`, error);
// }
//
...
//     }
// } catch (error) {
//     logger.error(`[name_convert.js] 读取或解析 ${AvatarIconDataFile} 时出错:`, error);
//     avatar_icon_data = {};
// }
//
// /**
...
//     } else {
//         // Full name not found for character ID: ${charIdStr}. Using default
...
//         }
//     }
//     //Sprite file not found for equipment ID: ${equipIdStr}.
...
//     } else {
//         // Short name not found for character ID: ${charIdStr}.
//         return undefined;
//     }
// }
//
...
//             return BASE_IMAGE_URL + iconPath;
//         } else {
//             //logger.debug(`[name_convert.js] ${AvatarIconDataFile} 中角色 ID ${charIdStr} 的 CircleIcon 路径不以 '/' 开头: "${iconPath}". 无法构建 URL.`);
//             return undefined;
//         }
//     } else {
//
